...
def default_profile_path() -> str:
    ...
    return os.path.join(pkg, "config", "ph_control_profiles.yaml")
...
class PhMotorInterface:
    def __init__(self, profile_name: str, profile: Dict[str, Any], driver_ns_override: str = ""):
        ...
        rospy.loginfo("[PH-IF] profile=%s driver_ns=%s", self.profile_name, self.driver_ns or "/")
    ...
    def resolve_joint_name(self, motor_id: int, joint_name_override: str = "") -> str:
        ...
        for j in joints:
            if not isinstance(j, dict):
                continue
            protocol = normalize_protocol(j.get("protocol", ""))
            if protocol not in ("PH", "PH_UDP", "SWINGARM"):
                continue
            ...
            if can_device_expect:
                if str(j.get("can_device", "")) != can_device_expect:
                    continue
        ...
        raise RuntimeError(
            f"无法在 {self.joints_param} 中找到 PH 关节: motor_id={hex(motor_id)} "
    ...
    def check_motor_in_profile(self, motor_id: int) -> None:
        ids = self.profile.get("ph_motor_ids", [])
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(description="PH 电机控制接口（多车 profile）")
    ...
    p.add_argument("--action", required=True,
                   ...
                   help="控制动作")
    p.add_argument("--motor-id", default="0x601", help="目标电机ID，支持十进制/十六进制")
...
def print_profiles(profiles: Dict[str, Dict[str, Any]]) -> None:
    print("可用 PH profiles:")
    for name, cfg in profiles.items():
        desc = cfg.get("description", "")
        dev = cfg.get("can_device", "")
        ids = cfg.get("ph_motor_ids", [])
        print(f"- {name}: {desc}")
        print(f"    can_device: {dev}")
        print(f"    ph_motor_ids: {ids}")
...
def main() -> int:
    ...
    try:
        ...
        print(f"[PH-IF] 加载 profiles 失败: {e}", file=sys.stderr)
        return 2
    ...
    if args.profile not in profiles:
        print(f"[PH-IF] 未找到 profile: {args.profile}", file=sys.stderr)
        print_profiles(profiles)
        return 2
    ...
    try:
        ...
        print(f"[PH-IF] motor-id 无法解析: {args.motor_id}", file=sys.stderr)
        return 2

    rospy.init_node("ph_motor_interface_cli", anonymous=True)

    iface = PhMotorInterface(args.profile, profiles[args.profile], args.driver_ns)
    ...
    try:
        ...
        if args.action == "mode":
            if args.value not in (0.0, 1.0):
                raise RuntimeError("mode 动作要求 --value 只能是 0(position) 或 1(velocity)")
        ...
        print(f"[PH-IF] 执行失败: {e}", file=sys.stderr)
        return 1
