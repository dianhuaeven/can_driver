...
## 4.1.1 Service: `set_zero_limit`（零点+限位）

- 默认服务名：`/can_driver_node/set_zero_limit`
- 类型：`can_driver/SetZeroLimit`

用途：
- 设置零点偏置（`zero_offset_rad`）
- 支持读取 URDF 限位（`use_urdf_limits=true`）
- 可将限位下发到电机物理寄存器（`apply_to_motor=true`，当前 PP 协议支持）
- 设置时会读取当前电机位置并与限位做匹配，越界则拒绝生效
...
zero_offset_rad: 0.0
min_position_rad: -1.57
max_position_rad: 1.57
