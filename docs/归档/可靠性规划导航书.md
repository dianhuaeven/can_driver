...
private:
    struct JointConfig {
        ...
        std::string canDevice;     // "canable0" / "canable1" ...
...
can_driver_node:
  ...
  joints:
    - { name: left_wheel,   motor_id: 0x141, protocol: MT, can_device: canable0, control_mode: velocity }
    - { name: right_wheel,  motor_id: 0x142, protocol: MT, can_device: canable0, control_mode: velocity }
    - { name: rotary_table, motor_id: 0x06,  protocol: PP, can_device: canable1, control_mode: position }
    - { name: larger_arm,   motor_id: 0x05,  protocol: PP, can_device: canable1, control_mode: position }
    - { name: smaller_arm,  motor_id: 0x13,  protocol: PP, can_device: canable1, control_mode: position }
    - { name: wrist_x,      motor_id: 0x14,  protocol: PP, can_device: canable1, control_mode: position }
    - { name: wrist_y,      motor_id: 0x15,  protocol: PP, can_device: canable1, control_mode: position }
    - { name: wrist_z,      motor_id: 0x16,  protocol: PP, can_device: canable1, control_mode: position }
    - { name: actuator,     motor_id: 0x17,  protocol: PP, can_device: canable1, control_mode: position }
```
...
- [ ] 7-1  恢复 `can_driver.yaml` 中的真实 `can_device`（canable0 / canable1）
- [ ] 7-2  用 `candump canable0` 监听原始 CAN 帧，确认节点发出刷新查询帧
